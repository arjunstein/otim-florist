...
class ProdukDetail extends Component
{
    ...
    public function mount($id, $slug, $product_name)
    {
        ...
        $this->slug = Cache::remember("product-slug-{$slug}", 60 * 60 * 6, function () use ($slug) {
            return Product::where('slug', $slug)->first();
        });

        $this->produk = Cache::remember("product-{$this->id}", 60 * 60 * 6, function () {
            return Product::with('category')->findOrFail($this->id);
        });

        // Cache categories with product counts
        $this->categories = Cache::remember('categories_with_count', 60 * 60 * 6, function () {
            return Category::withCount('product')->get();
        });

        // Cache product by category
        $this->products = Cache::remember("category-products-{$this->produk->category_id}", 60 * 60 * 6, function () {
