...
<template>
    <article class="group overflow-hidden rounded-2xl border bg-card shadow-xs transition-shadow duration-200 hover:shadow-md">
        <Link :href="`/products/${props.product.slug}`" class="block focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
            <div class="relative grid aspect-[4/3] place-items-center overflow-hidden bg-secondary/70">
                <svg class="size-24 text-primary/70 transition-transform duration-300 group-hover:scale-110 motion-reduce:transition-none" viewBox="0 0 40 40" fill="none" aria-hidden="true">
            ...
            <div class="p-4">
                <p class="text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">{{ props.product.category.name }}</p>
                <h2 class="mt-2 text-lg font-semibold tracking-tight">{{ props.product.name }}</h2>
                <p class="mt-3 font-semibold text-primary">{{ priceFormatter.format(props.product.price) }}</p>
            </div>
        </Link>
    </article>
</template>
